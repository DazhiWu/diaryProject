...
DO $$
BEGIN
  IF to_regclass('public.diary_image_paths') IS NULL OR to_regclass('private.diary_image_sequences') IS NULL THEN RAISE EXCEPTION 'migration tables are missing'; END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid = 'public."diaryContent"'::regclass AND tgname = 'diary_image_invariants_trigger' AND NOT tgisinternal AND (tgtype & 2) = 0) THEN RAISE EXCEPTION 'diary trigger is not AFTER'; END IF;
END $$;
