...
This is a personal diary application built with Next.js and Supabase. It supports diary CRUD, image/audio uploads, search/calendar views, health tracking, anonymous messages, CSV export, yearly summaries, and an admin-only private knowledge index plus cited on-demand recall answers. Server API routes call an ordered runtime-configured ModelScope chat-model list for title/emotion analysis, translation, and grounded answer generation. Local development uses the Qwen3-Embedding-0.6B FastAPI service for indexing and queries without reranking; deployed queries use Workers AI Embedding and reranking. Guest, viewer, and admin UI modes use simple password checks.

## Active knowledge-product boundary

The active product is the on-demand private diary recall assistant described in [`docs/PRIVATE_RECALL_ASSISTANT.md`](docs/PRIVATE_RECALL_ASSISTANT.md). Theme-timeline, observation-review, aggregation/comparison, and Ollama runtime code has been removed. Applied Phase 3A–3C migrations and dormant production `understanding_*` data remain for database-history preservation; the application must not read or write them. Unapplied Phase 3D/v4 SQL is archived outside `supabase/migrations/`. See [`docs/archive/phase3/README.md`](docs/archive/phase3/README.md).

`lib/knowledgeRecall.ts` owns ephemeral rule-based scope/direction planning; `lib/server/knowledgeRecall.ts` bounds multi-direction search and seven-day follow-up excerpt retrieval. Primary search runs before sequential expansion. Local `pnpm dev` uses normalized local query embeddings and vector order; production Workers AI has 20-second Embedding and eight-second reranking deadlines with classified safe errors. Optional transient retrieval failures retain existing evidence only with explicit partial-retrieval disclosure. Answers expose resolved scope/selected evidence counts and accept optional current-experience context. No additional planning model or persistent knowledge is created.
...
- ModelScope OpenAI-compatible API with an ordered server runtime model list; local Qwen3-Embedding-0.6B FastAPI service
...
- `app/layout.tsx` starts the global Liquid Glass visual mode. `DiaryAppShell` defaults to Liquid Glass and toggles the `liquid-glass` class on the document root for the original-page comparison mode; `app/globals.css` applies the active mode to shared primitives and portaled dialogs/selects/toasts without changing business behavior or layout.
- `HealthDateRangePicker` is shared by health-condition editing and CSV export. It accepts a start/end pair from 2024-11-01 onward, normalizes reverse-order selection, and supports a consumer-supplied maximum date; calendar health legends include only conditions overlapping the visible month.
...
- ModelScope-backed routes keep the token and ordered `MODELSCOPE_CHAT_MODEL` list server-side, enforce 50,000-character analysis/translation input and 500-character factual questions, use a 30-second timeout for analysis/translation and a 75-second timeout for each factual-answer model attempt, and share five interactive calls per IP per 60 seconds through `AI_RATE_LIMITER`. Factual answers request at most 800 Chinese characters and use a 2,000-token completion budget for ordinary models; configured `deepseek-ai/*` reasoning models receive 8,000 tokens so private reasoning can finish before the required final JSON. The application reads only final `content` and never exposes or logs `reasoning_content`. Explicit fallback is allowlisted: network/timeout, HTTP 404/408/410/425/429/5xx, explicit model-level provider codes, unreadable or malformed response bodies at the SDK boundary, missing/blank completion content, invalid analysis output, and invalid factual-answer JSON/citations advance. OpenAI SDK transport errors are classified by runtime type so Worker minification cannot hide abort/connection failures. Authentication/configuration/quota, ambiguous request-contract HTTP failures, database failures, local `HttpError`, and unknown program errors remain terminal with safe user feedback. Per-model safe logs record outcome and elapsed milliseconds without prompts, diary evidence, provider response text, or credentials. The download route returns safely escaped CSV with byte-accurate length.
...
- The admin-only knowledge base uses queued diary indexing, newline-first chunks, SHA-256 idempotency, 1024-dimensional Qwen3 embeddings, exact vector scans plus literal keyword fusion, optional date filters, and source-diary navigation. Local `pnpm dev` sends document and query inputs to `http://127.0.0.1:8000/embeddings`, normalizes query vectors, and intentionally skips reranking. The deployed Worker uses Workers AI Qwen3 query embeddings and BGE reranking. Both paths ask the existing Supabase RPC for 20 fused candidates, merge adjacent hits, and keep at most two independent results per diary. `rerankApplied` records the environment-dependent ranking path; no query vector is returned.
- Administrator factual answers reuse that search pipeline and pass at most eight bounded recall excerpts to the configured ModelScope chat models. After authorization, validation, and rate limiting, `/api/knowledge/answer` streams padded NDJSON start, progress, and heartbeat events every ten seconds before its final result or safe error event, preventing quiet multi-search/model-fallback requests from being canceled by an idle network intermediary. The browser displays the current retrieval, evidence-selection, generation, fallback, or citation-validation stage plus elapsed seconds; heartbeats repeat the latest stage. Progress contains only stage names, bounded counts, attempt position, and model ID, never the question, diary text, prompt, reasoning, provider body, error detail, or credential. The server assigns `S1`–`S8`, treats diary text as untrusted quoted data, and validates structured output plus inline citation IDs inside each model attempt. Expected JSON/citation-contract failures switch models without logging or displaying unvalidated text; unexpected parser/program errors remain terminal. Accepted IDs map back to server-owned metadata, and valid insufficient-evidence output returns the fixed non-answer. Zero-candidate requests do not contact or reserve ModelScope. Answers, citations, and conversations are not persisted.
- Historical Phase 3A–3C schema and derived rows remain dormant in production. The runtime route, UI, review/comparison logic, and Ollama integration are removed; no active code may depend on `understanding_*`. A future table deletion requires a separate backup and cleanup migration.
- One local-development administrator sync click runs sequential API batches of up to ten sources without a batch/source cap, stopping only when the queue is empty, three consecutive source failures occur, or a request fails. Individual tasks and batches remain at least two seconds apart. Production is status/search/factual-answer-only: it disables sync, rebuild, and retry controls, and production index-maintenance POST requests return `409`. While syncing locally, the status card bypasses caches and polls every two seconds; every exit path performs a final refresh. Indexed-source progress is the current completed-job count, not the historical non-null `last_indexed_at` count. A failed source is not automatically retried; processing continues until three consecutive failures stop the click and requeue the remaining claimed sources. Manual failed-job retry retains the existing tail-of-queue behavior. Direct knowledge search defaults to `2024-11-04` through the browser's local current date, while keeping both fields editable.
...
- The fixed `127.0.0.1` Embedding endpoint is maintained outside this repository and serves local document indexing plus local-development queries. Local query search intentionally skips the Workers AI binding and reranker. Deployed search requires the configured Workers AI `AI` binding and never attempts to reach loopback.
...
- `next.config.mjs` does not initialize OpenNext Cloudflare development bindings. Local query Embedding uses the loopback FastAPI service; runtime bindings remain available in the deployed OpenNext Worker.
- `lib/runtimeEnv.ts` must keep non-production `process.env` lookup ahead of `getCloudflareContext()`. Local `.env.local` values cover sessions, Supabase, and ModelScope without opening remote binding connections; missing local values may fall back to Cloudflare context, while production keeps bindings authoritative. Reversing this order adds one remote proxy attempt per environment-variable read and made ordinary local API requests wait several seconds.
...
- [`docs/archive/phase3/README.md`](docs/archive/phase3/README.md): archived Phase 3 designs, retirement boundary, and unapplied SQL.
