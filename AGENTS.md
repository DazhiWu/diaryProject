...
- Phase 3A/3B is implemented and operator-accepted locally as an administrator theme-timeline and observation-review slice; Phase 3C local development may begin. Its schema migrations are applied in production through `20260731071402_phase3_summary_regeneration`; least-privilege postflights, advisors, transactional mutation/regeneration smoke, rollback smoke, and the final operator review/regeneration workflow passed. Each run atomically verifies the approved 598-source corpus fingerprint, stores exact source IDs/hashes, processes the selected date range through frozen Ollama settings, and preserves proposed observations plus immutable chunk evidence. Extraction schemas enumerate only real chunk indexes and summary schemas only real observation IDs; strict server parsing remains the final boundary. The UI pages through one observation at a time and supports confirm, edit, and reject. Edits update the stable observation while appending before/after review history; edits or rejection move linked active summaries to history, and stale/incomplete runs cannot be reviewed. After every observation reaches confirmed/edited/rejected, the local-only regeneration action uses only confirmed/edited observations, makes one Ollama call when any remain (or emits a deterministic no-retained-observation statement when none remain), and creates a proposed summary linked to the previous history head; it never reruns diary extraction. Phase 3C must aggregate only confirmed/edited observations from completed/current, non-stale runs, compute exact statistics in PostgreSQL, disclose semantic coverage/version metadata, and preserve the full link to original evidence. It may use the reviewed monthly pilot and must not process the pending diary backlog merely to start development. Failed-source diagnostics remain bounded, credential-redacted, and administrator-only. The Worker source is not deployed or production-accepted yet.
