...
- A local Ollama server with `qwen3.5:4b` for Phase 3A extraction and summary generation; it is not a production Worker dependency.
...
| `lib/server/themeTimeline.ts` | Phase 3A frozen-corpus extraction, deterministic coverage, stale detection, summary generation, and review orchestration |
...
- Phase 3A source extraction and non-empty proposed-summary generation use local Ollama and do not reserve or consume ModelScope quota. Each local process request claims one frozen source. Ollama timeout/unavailability releases that source to pending and stops cleanly; invalid structured output is recorded as a retryable failed source.
...
- `/api/knowledge/understanding` is admin-only and additionally requires the Phase 3 migrations. GET reads stored runs without browser credentials. POST create/process/retry/regenerate-summary remains local-development-only; Origin-checked summary and observation review may run online for stored, non-stale results.
...
3. Before deploying knowledge routes to a new environment, apply `20260719155837_knowledge_base_index.sql`, then apply `20260720134848_modelscope_daily_quota.sql` to the intended Supabase project and review security/performance advisors. Production has both migrations applied.
   ...
   Before deploying `/api/knowledge/understanding`, confirm `20260730080402_phase3_theme_timeline`, `20260730081356_phase3_theme_timeline_fk_indexes`, `20260731024031_phase3_ollama_run_config`, `20260731063023_phase3_observation_review`, and `20260731071402_phase3_summary_regeneration` are present. Rerun the corresponding postflights if the target database changed and review both advisor classes. All five migrations are applied in production; the Worker route remains undeployed.
...
- Phase 3A/3B local development and operator acceptance are complete, so Phase 3C local development may begin without deploying the current Worker or processing the pending diary backlog. The base schema, Ollama configuration, observation-review, and summary-regeneration migrations are applied. Phase 3B postflights, confirm/edit/reject/regenerate transaction smokes, advisor comparisons, and rollback-script smokes passed on 2026-07-31. The operator completed all observations in the latest run, accepted the regenerated/edited summary history, and reported the final local workflow working normally after runtime lookup stopped opening unnecessary remote binding connections; authenticated read-only timing was 1.3–1.4 seconds on warm requests. Phase 3C migrations must be designed with rollback and least-privilege postflight first and require separate operator confirmation before production application. Do not create a billable Supabase branch or introduce any paid provider call for Phase 3C; use the existing reviewed monthly pilot and frozen 598-source development baseline. Still verify interrupted-run resume and duplicate prevention, provider-unavailable stop, isolated stale-source blocking, guest/viewer denial, citation navigation, exact PostgreSQL aggregation, Worker deployment, refreshed full-corpus coverage, and production acceptance.
