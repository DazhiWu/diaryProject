...
- Fact Layer: submit an administrator question with and without matching diary evidence. Confirm supported answers contain only server-owned `S1`–`S5` citations whose cards open the correct source diaries; zero or valid model-declared inadequate evidence produces the explicit non-answer; reranker fallback is visible; duplicate submission is blocked; and guest/viewer/invalid-Origin requests are denied. Malformed provider JSON or citation IDs must switch to the next model without displaying/logging unvalidated text; complete allowlisted exhaustion returns `所有模型 API 调用失败`. Confirm no answer, citation, or conversation row is written.
