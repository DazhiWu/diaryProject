...
- Oversized JSON/multipart requests return `413`; invalid dates, field lengths, file types, or arrays return `400` before downstream writes. ModelScope analysis and translation attempts time out after 30 seconds; factual-answer attempts use 45 seconds because grounded JSON generation is longer. SDK abort, connection-timeout, and connection errors are identified by runtime type so minified Worker class names do not prevent ordered fallback. The sixth interactive AI/translation/knowledge-search/factual-answer call in 60 seconds returns `429` for the same client IP.
