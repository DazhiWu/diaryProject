...
- `/api/knowledge/understanding` is admin-only and additionally requires the Phase 3A migration. GET reads stored runs without browser credentials. POST create/process/retry remains local-development-only; Origin-checked summary review may run online for stored, non-stale results.
...
3. Before deploying knowledge routes to a new environment, apply `20260719155837_knowledge_base_index.sql`, then apply `20260720134848_modelscope_daily_quota.sql` to the intended Supabase project and review security/performance advisors. Production has both migrations applied.
   ...
   Before deploying `/api/knowledge/understanding`, confirm `20260730080402_phase3_theme_timeline`, `20260730081356_phase3_theme_timeline_fk_indexes`, and `20260731024031_phase3_ollama_run_config` are present. Rerun the corresponding postflights if the target database changed and review both advisor classes. All three migrations are applied in production; the Worker route remains undeployed.
...
- Phase 3A remaining pre-production work: the base production schema postflight and transaction-only 598-source create/snapshot/claim/release rollback smoke passed on 2026-07-30. The Ollama configuration migration, least-privilege checks, unchanged advisors, and rollback subtransaction smoke passed on 2026-07-31. Windows-to-WSL `/api/tags` returned `qwen3.5:4b`; a one-source 2026-07-22 run froze the requested config and completed one extraction plus one proposed summary with no failed/pending/stale run sources. Still verify interrupted-run resume and duplicate prevention; force one invalid structured output and one provider-unavailable stop in an isolated environment; mutate an indexed source only in isolation to confirm stale blocking; and complete guest/viewer denial, administrator citation navigation, exact PostgreSQL month distribution, review-history, full rollback, Worker deployment, and production acceptance checks.
