...
- Phase 3A source extraction and non-empty proposed-summary generation also reserve from the same 180-call Beijing-day ModelScope budget. Each local process request claims one frozen source. Quota denial releases that source to pending and pauses cleanly; it must not be counted as failed or processed.
...
3. Before deploying knowledge routes to a new environment, apply `20260719155837_knowledge_base_index.sql`, then apply `20260720134848_modelscope_daily_quota.sql` to the intended Supabase project and review security/performance advisors. Production has both migrations applied.
   ...
   Before deploying `/api/knowledge/understanding`, confirm `20260730080402_phase3_theme_timeline` and `20260730081356_phase3_theme_timeline_fk_indexes` are present, rerun both Phase 3A postflights if the target database changed, and review both advisor classes. Production applied both migrations and passed both postflights on 2026-07-30; the Worker route remains undeployed.
...
- Phase 3A remaining pre-production work: the production schema postflight and transaction-only 598-source create/snapshot/claim/release rollback smoke passed on 2026-07-30, with no stored derived rows or ModelScope calls. From local development, create a small date-range theme run and confirm only range-eligible sources enter processing. Interrupt and resume after at least one source; verify no duplicate observation/evidence rows. Force one failure and one quota stop, verifying failed versus pending counts and safe retry. Edit one indexed source only in an isolated test environment and verify the run becomes stale and cannot be confirmed. Confirm guest/viewer denial, administrator citation navigation, exact PostgreSQL month distribution, and confirm/edit/reject/supersede history. Run the full rollback only in an isolated environment and verify original diary and knowledge-chunk counts are unchanged.
