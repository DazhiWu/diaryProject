...
- A local Ollama server with `qwen3.5:4b` for Phase 3A extraction/summary and Phase 3D period-comparison generation; it is not a production Worker dependency.
...
| `lib/server/themeTimeline.ts` | Phase 3A–3D frozen-corpus extraction, deterministic aggregation, stale detection, summary/period-comparison generation, and review orchestration |
| `lib/server/ollamaClient.ts` | Server-only native Ollama chat client with structured output and bounded generation controls |
...
| `supabase/migrations/20260730071934_phase3_theme_timeline.sql` | Applied Phase 3A derived understanding schema and lifecycle RPCs |
| `supabase/migrations/20260730081229_phase3_theme_timeline_fk_indexes.sql` | Applied Phase 3A summary/evidence-link foreign-key covering indexes |
| `supabase/migrations/20260731015350_phase3_ollama_run_config.sql` | Applied Phase 3A per-run Ollama configuration extension |
| `supabase/migrations/20260731062228_phase3_observation_review.sql` | Applied Phase 3B observation review/history and linked-summary invalidation |
| `supabase/migrations/20260731070803_phase3_summary_regeneration.sql` | Applied reviewed-observation-only local summary regeneration |
| `supabase/migrations/20260803021419_phase3_corpus_aggregation.sql` | Applied Phase 3C deterministic aggregate/version tables and service-role RPCs; production version `20260803025058_phase3_corpus_aggregation` |
| `supabase/migrations/20260803034844_phase3_change_contradiction_analysis.sql` | Unapplied Phase 3D versioned period-comparison/finding/review tables and service-role RPCs; must precede any 3D Worker deployment |
| `supabase/migrations/20260803082412_phase3_theme_semantic_quality_v4.sql` | Unapplied Phase 3A v4 ThemeSpec, exact sentence-evidence, uncertain-scope, and review-before-first-summary extension; must precede matching v4 source deployment |
...
| `OLLAMA_BASE_URL` | `lib/server/ollamaClient.ts` | For local Phase 3A processing | No credential, but trusted server-only configuration | Local Next.js runtime only; defaults to `http://127.0.0.1:11434` |
...
- ModelScope analysis, translation, and factual-answer generation share a Supabase-backed limit of 180 upstream HTTP attempts per Beijing calendar day. Every attempted model reserves one slot immediately before its HTTP request; quota denial/unavailability stops fallback. Timeout, network, and HTTP non-2xx failures advance to the next model. Empty content, invalid analysis JSON, and malformed factual-answer JSON/citations return immediately without another model attempt. Zero-candidate answers, local document Embedding, and Workers-AI-only knowledge search do not consume this counter.
- Phase 3A source extraction and non-empty proposed-summary generation use local Ollama and do not reserve or consume ModelScope quota. In v4 each local process request claims one source, makes one complete sentence-partition call, and makes a second synthesis call only when relevant/uncertain evidence remains. A run does not auto-summarize after extraction; after every observation reaches confirmed/edited/rejected, the first non-empty summary uses one local call. Ollama timeout/unavailability releases that source to pending and stops cleanly; malformed, incomplete, overlapping, or unknown-ID partitions are bounded retryable failures.
- Never accept an Ollama base URL from the browser. If Next.js runs in WSL while Ollama runs on Windows, Windows loopback is not necessarily WSL loopback: configure Ollama to listen on a WSL-reachable Windows interface, restrict TCP 11434 to the local/WSL virtual network in Windows Firewall, and set `.env.local` to `OLLAMA_BASE_URL=http://<Windows-host-IP>:11434`. Never expose the unauthenticated Ollama API to the public internet. Confirm `curl "$OLLAMA_BASE_URL/api/tags"` lists the intended model before processing.
...
- `/api/knowledge/understanding` is admin-only and additionally requires the matching Phase 3 migrations. GET reads stored runs, aggregate history, and comparison history without browser credentials. During source-before-migration development, only PostgREST `PGRST205` for the optional 3D comparison or v4 semantic tables degrades to empty legacy metadata; permission, connectivity, malformed-row, and all other relation failures remain fatal. POST create/process/retry/regenerate-summary/generate-comparison remains local-development-only; Origin-checked summary/observation/comparison-finding review and deterministic aggregate regeneration may run online for stored, non-stale results. Before `20260803082412_phase3_theme_semantic_quality_v4.sql` is applied, new v4 creation fails explicitly with `409` while stored legacy runs remain readable. Aggregate regeneration performs one PostgreSQL transaction and no provider call; comparison generation makes one local Ollama call over the full reviewed observation set from two months in one aggregate.
- Configured Phase 3A creation additionally requires migration `20260731015350_phase3_ollama_run_config.sql`, applied to production as `20260731024031_phase3_ollama_run_config`. The API validates and stores model, `num_ctx`, temperature, `top_p`, `top_k`, thinking, output limits, and custom prompts. Server-owned evidence, anti-injection, and JSON-schema constraints are always appended and cannot be disabled from the UI.
- Apply the unapplied migrations in timestamp order: Phase 3D `20260803034844...` precedes the v4 quality migration `20260803082412...`. The v4 SQL alone passed an isolated, unlinked Supabase PostgreSQL 17 migration/postflight/functional/real-rollback/reapply cycle on 2026-08-03, including relevant/uncertain/irrelevant persistence and review-filtered first-summary finalization; every fixture transaction rolled back. This local result does not replace the target-project gate: apply through v4 inside the normal production migration flow, rerun the read-only postflight, then run both advisor classes before deploying matching Worker source. Finally verify guest `401`, viewer `403`, admin GET/review, production local-only mutation `409`, and a small local 4B pilot. Existing v3 test runs may be deleted later as authorized disposable derived data, but deletion is not required for migration correctness.
- Analysis, translation, and factual-answer generation require `reserve_modelscope_api_call()` before every ModelScope HTTP attempt. The answer route first reuses the Workers AI/Supabase retrieval pipeline; zero candidates return insufficient evidence without a reservation, while non-empty retrieval reserves once for each model actually attempted. Local document indexing calls `http://127.0.0.1:8000/embeddings`; standalone online knowledge search uses Workers AI query Embedding and reranking without reserving ModelScope quota.
...
3. Before deploying knowledge routes to a new environment, apply `20260719155837_knowledge_base_index.sql`, then apply `20260720134848_modelscope_daily_quota.sql` to the intended Supabase project and review security/performance advisors. Production has both migrations applied.
   ...
   The currently deployed `/api/knowledge/understanding` requires `20260730080402_phase3_theme_timeline`, `20260730081356_phase3_theme_timeline_fk_indexes`, `20260731024031_phase3_ollama_run_config`, `20260731063023_phase3_observation_review`, `20260731071402_phase3_summary_regeneration`, and `20260803025058_phase3_corpus_aggregation`; all six are applied in production. Before deploying source that reads Phase 3D comparisons, first apply source migration `20260803034844_phase3_change_contradiction_analysis.sql` under its production history version, run `20260803_phase3_change_contradiction_analysis_postflight.sql`, execute transaction generation/version/stale/rollback smokes, and review both advisor classes. The 3D migration has not been applied and the current Worker does not include 3D.
...
- ModelScope ordered fallback and shared quota: confirm models are attempted in `MODELSCOPE_CHAT_MODEL` order; a timeout/network/HTTP failure advances and reserves another slot; the first success stops; an empty or invalid successful response does not advance; and complete exhaustion returns `所有模型 API 调用失败`. Confirm reservation 180 succeeds, reservation 181 returns a clear `429` without an upstream call, counter/RPC errors return `503` without an upstream call, and the date rolls over at Beijing midnight. Verify analysis, translation, and non-empty factual answers share the counter; zero-candidate answers, local indexing, and standalone Workers AI knowledge search must not change it.
...
- Fact Layer: submit an administrator question with and without matching diary evidence. Confirm supported answers contain only server-owned `S1`–`S5` citations whose cards open the correct source diaries; zero or inadequate evidence produces the explicit non-answer; reranker fallback is visible; duplicate submission is blocked; guest/viewer/invalid-Origin requests are denied; and malformed provider JSON or citation IDs return `模型返回结果格式错误` without displaying unvalidated text or trying another model. Confirm no answer, citation, or conversation row is written.
- The Fact Layer acceptance matrix above was completed in production on 2026-07-30. Phase 3 development uses the separately documented frozen 598-source index checkpoint; the growing pending backlog is intentionally deferred until Phase 3 feature completion and does not require another Worker deployment.
- Phase 3C production rollout passed on 2026-08-03. Supabase migration `20260803025058_phase3_corpus_aggregation`, least-privilege postflight, both advisor classes, reviewed-pilot transaction generation, observation-drift detection, superseding-version logic, real rollback statements, and aggregate → observation → evidence → diary navigation passed. A rollback-only observation edit produced `observation_snapshot_changed` and then restored an empty stale set. Worker `cd1f5f09-a607-4781-8be0-36eba039762f` returned homepage `200`, guest knowledge `401`, viewer knowledge `403`, admin read/regenerate `200`, and production extraction `409`. Online regeneration preserved the first aggregate as history and produced a current 31/31-source, 16-observation, 16-diary, one-period aggregate with no stale reasons. The pending diary backlog was not processed and no model/provider call was made. Refreshed full-corpus coverage/regeneration and final Phase 3 acceptance remain pending.
- Phase 3D plus the Phase 3A irrelevant-result regression source verification passes 218 tests across 43 files, lint, strict TypeScript, Next.js production build, OpenNext Cloudflare build, Wrangler `4.108.0` dry-run, and `git diff --check`. Production migration/postflight/advisors, PostgreSQL transaction and real rollback smokes, Worker deployment, guest/viewer/admin role acceptance, and real two-month comparison/review remain pending. The current accepted aggregate contains only one nonempty month, so do not fabricate a second period or process pending diaries solely to exercise 3D.
...
- Oversized JSON/multipart requests return `413`; invalid dates, field lengths, file types, arrays, or Ollama controls return `400` before downstream writes. Each ModelScope model attempt times out after 30 seconds; local Ollama requests time out after 120 seconds. The sixth interactive AI/translation/knowledge-search/factual-answer call in 60 seconds returns `429` for the same client IP.
...
Version history, current deployment, and rollback capability were verified on 2026-08-03 with `pnpm exec wrangler deployments list --name diaryproject`. The current Phase 3C Worker version is `cd1f5f09-a607-4781-8be0-36eba039762f`; its immediate source/runtime rollback target is `b899d692-5607-4a11-83bf-e8ae952afdf2`. The earlier accepted Fact Layer target remains `b258ef8d-e303-42f2-941b-2dd72a2391ea`. Roll back the Worker before removing database objects. The unapplied Phase 3D rollback `supabase/rollbacks/20260803034844_phase3_change_contradiction_analysis_rollback.sql` removes only comparison functions/tables and preserves all 3A–3C/source records; its production execution smoke remains pending. Phase 3C rollback `supabase/rollbacks/20260803021419_phase3_corpus_aggregation_rollback.sql` removes only aggregate tables/functions and preserves every 3A/3B/source record; its exact drop statements passed inside a rollback-only transaction. The Phase 3A rollback `supabase/rollbacks/20260730071934_phase3_theme_timeline_rollback.sql` removes only the derived `understanding_*` functions and tables. The follow-up index and Ollama configuration rollbacks remove only their extensions after dependent source is rolled back. The Phase 3B observation-review rollback `supabase/rollbacks/20260731062228_phase3_observation_review_rollback.sql` first restores every reviewed observation and invalidated summary to its earliest pre-migration state, then removes only the review function/tables and restores the previous summary-review function; its full transaction smoke passed. The later `supabase/rollbacks/20260731070803_phase3_summary_regeneration_rollback.sql` removes only the local regeneration RPC and preserves already compatible summary history; its function-drop transaction smoke passed. The quota rollback deletes only its reservation RPC and usage table, but must not run while deployed source still depends on it. The separate knowledge-index rollback removes derived knowledge settings, chunks, and jobs while preserving source diaries and the vector extension.
