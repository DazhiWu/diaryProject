...
- Oversized JSON/multipart requests return `413`; invalid dates, field lengths, file types, or arrays return `400` before downstream writes. Each ModelScope model attempt times out after 30 seconds. The sixth interactive AI/translation/knowledge-search/factual-answer call in 60 seconds returns `429` for the same client IP.
