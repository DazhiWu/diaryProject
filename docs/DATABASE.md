...
- Private knowledge search: administrator-only POST APIs create a query embedding, request up to 20 candidates from the service-role-only hybrid RPC, apply date and private-scope filters, and fuse cosine similarity with literal Chinese/exact-text matches. The server merges adjacent candidate chunks in source order, removes duplicated overlap from the returned content, preserves the best rank/similarity, and limits each diary to at most two independent results before applying the requested result limit. Results include source date/title, merged chunk range, character offsets, and diary ID for traceability.
