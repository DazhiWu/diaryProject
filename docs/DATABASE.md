...
- Every ModelScope analysis/translation HTTP attempt reserves before sending. Quota database errors fail closed, and OpenAI SDK automatic retries are disabled so a single reservation cannot silently create multiple provider attempts.
- Local knowledge Embedding requests do not reserve or consume ModelScope quota.
