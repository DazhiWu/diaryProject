...
## Active recall access pattern

The [private recall assistant](PRIVATE_RECALL_ASSISTANT.md) reuses `search_private_knowledge()` through the existing server search module. It makes at most five date-bounded searches per answer and does not add tables or change RPCs, RLS, grants, storage, or existing indexing eligibility. Follow-up reading is retrieval of indexed excerpts, not unrestricted diary reads. Answers, plans, context, and citations are not persisted. The retired Phase 3 runtime does not participate in recall requests or writes.
...
## Dormant Phase 3 derived schema

Production migrations through Phase 3C were previously applied and created private `understanding_*` run, source, observation, evidence, review, summary, and aggregate tables plus service-role-only functions. Their historical rows remain dormant: the application no longer exposes `/api/knowledge/understanding`, no browser or server runtime path reads or writes these objects, and Ollama is no longer a project dependency.

The applied migrations, rollbacks, and postflights remain under `supabase/` so the existing production schema history is reproducible. RLS, deny-by-default browser access, and current grants remain unchanged. Do not delete these tables manually. A future removal requires an exported backup, a dependency inventory, and a new reviewed cleanup migration.

Phase 3D comparison and semantic-quality v4 were never applied. Their migration, rollback, and verification SQL files are preserved under [`docs/archive/phase3/sql-unapplied`](archive/phase3/sql-unapplied/) and are intentionally outside the active migration runner. Historical design context is in [`docs/archive/phase3/README.md`](archive/phase3/README.md).
...
- Private knowledge search: administrator-only POST APIs create a normalized 1024-dimensional query embedding locally during `pnpm dev` and through Workers AI after deployment. Both request exactly 20 candidates from the existing service-role-only hybrid RPC. Local development intentionally returns vector-ordered candidates with `rerankApplied: false`; production applies the Workers AI BGE reranker and falls back to vector order on reranker failure. The server then merges adjacent chunks, removes duplicated overlap, and limits each diary to at most two independent results. Diagnostic responses never return the query vector.
...
- Active database migrations, rollbacks, and postflights remain under `supabase/`; this repository is not a complete migration chain.
- `supabase/migrations/20260719155837_knowledge_base_index.sql` and its rollback/postflight define the active private knowledge index.
- `supabase/migrations/20260720134848_modelscope_daily_quota.sql` and its rollback define the active shared ModelScope call budget.
- Applied Phase 3A–3C migration, rollback, and verification files remain in `supabase/` only to preserve production history and support a future coordinated cleanup.
- Never-applied Phase 3D/v4 SQL is archived under `docs/archive/phase3/sql-unapplied/` and must not be moved back into the active migration chain without a new product decision and full review.
- `test_extra/CREATE_HEALTH_CONDITIONS_TABLE.sql` and `test_extra/CREATE_ANONYMOUS_MESSAGE_TABLE.sql` remain partial historical helpers.
...
- dormant Phase 3A–3C tables remain inaccessible to browser roles and outside all current application request paths;
