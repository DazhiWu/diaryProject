...
- Keep OpenAI SDK retries disabled. Factual-answer generation uses a 75-second timeout for variable cross-region latency, requests at most 800 Chinese characters, gives ordinary models 2,000 completion tokens, and gives configured `deepseek-ai/*` reasoning models 8,000 so they can emit final JSON; analysis and translation retain 30 seconds.
