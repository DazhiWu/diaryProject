...
**Status:** Storage and diary/AI phases completed on 2026-07-15; health table lockdown awaits separate approval. All three media buckets are private, direct anon diary/AI and Storage access is denied, and authorized application paths passed production regression.
...
|---|---|
| Production approver | DazhiWu — Storage phase approved and completed; each application-table domain requires separate approval |
...
| Pre-migration regression record | Batch 4 production regression passed 2026-07-15. Batch 5 pre-change run `2ef26a54-0c79-49ff-9a54-e46397dab65b` exited `0`; Storage intermediate and final runs also exited `0`. |
...
- Before requesting diary/AI approval, a 2026-07-15 08:02 UTC read-only ACL audit checked all ten remaining target tables. Every table is owned by `postgres`; every anon/authenticated/service_role table privilege has the sole grantor `postgres`; and the Supabase SQL execution role is the same non-superuser `postgres`. This removes the cross-grantor failure mode found in Storage. The diary/AI tables still have RLS enabled, the exact reviewed permissive policy, and CRUD privileges for anon/authenticated/service_role. Their reviewed forward migration and rollback are executable by the current role, and the postflight fails closed on any residual anon/authenticated access or lost service-role CRUD. Diary/AI mutation still requires separate approval.
- DazhiWu approved the diary/AI domain. Migration record `20260715080441 batch5_diary_ai_rls` applied and its fail-closed postflight passed. Behavior run `cdce7dd8-b0c1-406d-a83e-01e082b78f29` confirmed anon SELECT/INSERT returned `401` for both tables; guest/viewer/admin diary APIs returned `200`; AI reads remained guest `401`, viewer `403`, admin `200`; cleanup completed; and the admin CSV export returned `200 text/csv`. Local tests passed 10 files/42 tests. Independent verification found zero tagged rows, two RLS-enabled tables, zero policies, zero anon/authenticated ACL entries, and 16 service-role ACL entries granted by `postgres`. Wrangler control-plane reads failed twice with `fetch failed`, and one Supabase migration-list call returned HTTP 520, while the production Worker behavior and SQL channel remained healthy; the migration table independently confirmed the record. No Worker deployment occurred. Diary/AI is complete; health remains unapplied.
