...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string; sequence: string }> }) {
  try { await assertAllowedOrigin(request); requireAdmin(await readSession(request.headers.get('cookie'))); const value = await params; const id = numberFrom(value.id, 'diary id'); const { supabase, path } = await diaryPath(id, numberFrom(value.sequence, 'sequence')); const form = await readFormDataBody(request, REQUEST_LIMITS.imageForm); const file = fileField(form.get('file'), { maximumBytes: FIELD_LIMITS.diaryImageBytes, mimeType: 'image/webp' }); await (await createSupabaseUploadStore()).upload(DIARY_BUCKET, path, file, { upsert: true, contentType: 'image/webp' }); const { error } = await supabase.from('diaryContent').update({ modifiedAt: new Date().toISOString() }).eq('id', id); if (error) throw new Error('Diary image metadata write failed'); return NextResponse.json({ ok: true, path }) } catch (error) { return responseFor(error) }
}
