...
export async function GET(request: Request) {
  try {
    ...
    if (start || end) {
      ...
      const { data, error } = await (await getSupabaseAdmin()).from('diaryContent').select('id, date, subtitle, content, image_paths, modifiedAt, created_at').gte('date', start).lte('date', end).order('date', { ascending: true })
...
export async function POST(request: Request) {
  try {
    ...
    if (!body || typeof body.date !== 'string' || typeof body.content !== 'string' || typeof body.subtitle !== 'string' || !Array.isArray(body.image_paths)) throw new HttpError(400, 'Invalid diary')
    const { data, error } = await (await getSupabaseAdmin()).from('diaryContent').insert([{
      date: body.date, content: body.content, subtitle: body.subtitle, image_paths: body.image_paths, modifiedAt: new Date().toISOString(),
