...
export function validateAuthConfiguration(configuration: AuthConfiguration): asserts configuration is { viewer: string; admin: string } {
  const { viewer, admin } = configuration
  if (!viewer) {
