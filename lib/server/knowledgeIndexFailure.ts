...
export type KnowledgeIndexFailureCategory =
  ...
  | 'modelscope_auth'
  | 'modelscope_network'
  | 'modelscope_rate_limit'
  | 'modelscope_request'
  | 'modelscope_quota_stop'
  | 'modelscope_timeout'
  | 'modelscope_upstream'
  | 'source_not_found'
  | 'source_query_failed'
...
export function sanitizeKnowledgeIndexFailure(error: unknown, diaryContent?: string): FailureDetails & { storedMessage: string } {
  ...
  if (error instanceof KnowledgeIndexStepError) {
    ...
    else if (status === 429) category = 'modelscope_rate_limit'
    else if (status === 401 || status === 403) category = 'modelscope_auth'
    else if (status !== undefined && status >= 500) category = 'modelscope_upstream'
    else if (status !== undefined && status >= 400) category = 'modelscope_request'
    else if (/timeout|abort/iu.test(name) || /timed?\s*out/iu.test(message)) category = 'modelscope_timeout'
    else if (/connection|network|fetch/iu.test(name) || ['ECONNREFUSED', 'ECONNRESET', 'ENOTFOUND'].includes(code ?? '')) category = 'modelscope_network'
  }
