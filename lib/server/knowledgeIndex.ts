...
export type KnowledgeIndexExecutionMode = 'local' | 'status-only'
...
export type KnowledgeIndexStatus = {
  executionMode: KnowledgeIndexExecutionMode
  totalSources: number
...
export function getKnowledgeIndexExecutionMode(): KnowledgeIndexExecutionMode {
  return process.env.NODE_ENV === 'production' ? 'status-only' : 'local'
}
...
export async function getKnowledgeIndexStatus(): Promise<KnowledgeIndexStatus> {
  ...
  return {
    executionMode: getKnowledgeIndexExecutionMode(),
    totalSources,
    indexedSources: completed,
    totalChunks,
    pending,
    processing,
    failed,
    completed,
    excluded,
    lastIndexedAt: latest.data?.last_indexed_at ?? null,
  }
}
...
type KnowledgeIndexProcessDependencies = {
  getSupabase: typeof getSupabaseAdmin
  embedTexts: typeof embedKnowledgeTexts
  getStatus: typeof getKnowledgeIndexStatus
  wait: (milliseconds: number) => Promise<void>
}

async function failJobs(supabase: Awaited<ReturnType<typeof getSupabaseAdmin>>, sourceIds: number[], message: string): Promise<void> {
  if (sourceIds.length === 0) return
  const { error } = await supabase.from('knowledge_index_jobs').update({ status: 'failed', last_error: message.slice(0, KNOWLEDGE_INDEX_LAST_ERROR_MAX_CHARS), updated_at: new Date().toISOString() }).in('source_id', sourceIds)
  if (error) console.error('[knowledge-index]', { operation: 'mark-failed', outcome: 'failed' })
}

async function requeueJobs(supabase: Awaited<ReturnType<typeof getSupabaseAdmin>>, sourceIds: number[], message: string): Promise<void> {
  if (sourceIds.length === 0) return
  const now = new Date().toISOString()
  const { error } = await supabase.from('knowledge_index_jobs').update({
    status: 'pending',
    queued_at: now,
    started_at: null,
    completed_at: null,
...
export async function processKnowledgeIndexBatch(
  batchSize = 10,
  initialConsecutiveFailures = 0,
  dependencyOverrides: Partial<KnowledgeIndexProcessDependencies> = {},
): Promise<KnowledgeIndexBatchResult> {
  const dependencies: KnowledgeIndexProcessDependencies = {
    getSupabase: getSupabaseAdmin,
    embedTexts: embedKnowledgeTexts,
    getStatus: getKnowledgeIndexStatus,
    wait,
    ...dependencyOverrides,
  }
  ...
  if (claimedIds.length === 0) return { processed: 0, failed: 0, consecutiveFailures, stoppedForConsecutiveFailures: false, status: await dependencies.getStatus() }
  ...
  if (diaryError || settingError || chunkError) {
    ...
    await failJobs(supabase, claimedIds, failure.storedMessage)
  ...
  for (const [sourceIndex, sourceId] of claimedIds.entries()) {
    if (sourceIndex > 0) await dependencies.wait(KNOWLEDGE_INDEX_TASK_INTERVAL_MS)
    ...
    if (!diary || !setting) {
      ...
      await failJobs(supabase, [sourceId], failure.storedMessage)
      failed += 1
      ...
      if (consecutiveFailures >= KNOWLEDGE_INDEX_CONSECUTIVE_FAILURE_LIMIT) {
        ...
        await requeueJobs(supabase, claimedIds.slice(sourceIndex + 1), stop.storedMessage)
    ...
    try {
      ...
      const embeddings = await dependencies.embedTexts(chunks.map((chunk) => diary.subtitle ? `${diary.subtitle}\n${chunk.content}` : chunk.content), 'document')
  ...
  return { processed, failed, consecutiveFailures, stoppedForConsecutiveFailures, status: await dependencies.getStatus() }
}
